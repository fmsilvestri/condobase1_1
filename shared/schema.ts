...
export const hospedagens = pgTable("hospedagens", {
  ...
  bloco: varchar("bloco"),
  nomeHospede: varchar("nome_hospede").notNull(),
  telefoneHospede: varchar("telefone_hospede"),
  emailHospede: varchar("email_hospede"),
  documentoHospede: varchar("documento_hospede"),
  quantidadeHospedes: integer("quantidade_hospedes").default(1),
  plataforma: varchar("plataforma"),
  codigoReserva: varchar("codigo_reserva"),
  dataCheckIn: timestamp("data_check_in").notNull(),
  dataCheckOut: timestamp("data_check_out").notNull(),
  placaVeiculo: varchar("placa_veiculo"),
  modeloVeiculo: varchar("modelo_veiculo"),
  corVeiculo: varchar("cor_veiculo"),
  observacoes: text("observacoes"),
  status: varchar("status").notNull().$type<StatusHospedagem>().default("reservado"),
  boasVindasEnviadas: boolean("boas_vindas_enviadas").default(false),
  dataEnvioBoasVindas: timestamp("data_envio_boas_vindas"),
  urlVideoExplicativo: varchar("url_video_explicativo"),
  urlRegimentoInterno: varchar("url_regimento_interno"),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});
