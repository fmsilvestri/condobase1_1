...
- **Gestão de Atividades**: Activity management module with template-based workflow system. Features include: reusable activity templates (filtered by function and periodicity), per-employee template assignment for building daily/weekly work flows, batch sending via WhatsApp with PDF generation, history tracking of sent lists, template CRUD (create, edit, delete with confirmation), and completion statistics. The "Enviar em Lote" tab allows expanding each employee to assign specific templates, with a summary section showing all assignments before sending.
...
- **Marketplace**: Service marketplace connecting residents with approved providers. Features include service categories, provider registration with admin approval, service offers with pricing (fixed, hourly, quote-based, negotiable), contracting workflow (solicitado → confirmado → em_execucao → concluido), reviews/ratings system, and automated WhatsApp notifications via Twilio for contracting events. Role-based access: residents browse/contract services, providers manage offers and handle requests via dedicated dashboard (/marketplace-fornecedor), admins manage categories/services and approve providers.
